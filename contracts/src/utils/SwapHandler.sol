...
contract SwapHandler is SwapActions, IUnlockCallback, ISwapHandler {
    ...
    address public immutable najRouter;

    /// @notice NajHook contract
    INajLaunchpad public immutable najLaunchpad;

    /// @notice NajHook contract
    INajHook public immutable najHook;
    ...
    constructor(
        ...
        require(_najLaunchpad != address(0), "Invalid NajLaunchpad address");
        require(_poolManager != address(0), "Invalid PoolManager address");
        require(_najRouter != address(0), "Invalid Router address");

        TEE = _tee;
        najRouter = _najRouter;
        najHook = INajHook(_najHook);
        poolManager = IPoolManager(_poolManager);
        najLaunchpad = INajLaunchpad(_najLaunchpad);
    }
    ...
    function postBatch(PoolId poolId, bytes calldata strategyUpdateParams, SwapData[] calldata swaps) external onlyTEE {
        ...
        if (strategyParams.length > 0) {
            IStrategyAdapter(najLaunchpad.getLaunchConfig(poolId).strategyAdapter).update(strategyParams, priceUpdate);
