...
import {INajHook} from "@interfaces/INajHook.sol";
...
/// @dev Enables market makers to launch and manage their own proprietary pools with NajHook
contract NajLaunchpad is Ownable, IUnlockCallback, INajLaunchpad {
    ...
    /// @notice NajHook contract instance
    INajHook public najHook;
    ...
    error NajLaunchpad__InvalidTokens();
    error NajLaunchpad__InvalidStrategy();
    error NajLaunchpad__PoolAlreadyExists();
    error NajLaunchpad__InvalidAsset();
    error NajLaunchpad__InsufficientAmount();
    error NajLaunchpad__Unauthorized();
    error NajLaunchpad__NajHookAlreadySet();
    error NajLaunchpad__InvalidThresholdAdapter();
    error NajLaunchpad__InvalidStrategyAdapter();
    error NajLaunchpad__InvalidPoolManager();
    ...
    constructor(address _owner, address _poolManager) Ownable(_owner) {
        if (_poolManager == address(0)) revert NajLaunchpad__InvalidPoolManager();
    ...
    function setNajHook(address _najHook) external onlyOwner {
        if (address(najHook) != address(0)) revert NajLaunchpad__NajHookAlreadySet();
        najHook = INajHook(_najHook);
    }
    ...
    function launch(LaunchConfig calldata _launchConfig) external returns (PoolId poolId) {
        ...
        if (bytes(launchConfigs[poolId].poolName).length != 0) {
            revert NajLaunchpad__PoolAlreadyExists();
        }

        IPoolManager(POOL_MANAGER).initialize(key, 7922816251426433759354395);

        najHook.configurePool(key, _launchConfig.strategyAdapter, _launchConfig.thresholdAdapter);
    ...
    function addLiquidity(PoolId poolId, address asset, uint256 amount) public payable onlyCurator(poolId) {
        ...
        if (asset != config.token0 && asset != config.token1) revert NajLaunchpad__InvalidAsset();
        ...
        if (asset != address(0)) {
            ...
            if (msg.value != amount) revert NajLaunchpad__InsufficientAmount();
        }
    ...
    function unlockCallback(bytes calldata rawData) external override returns (bytes memory) {
        if (msg.sender != POOL_MANAGER) revert NajLaunchpad__Unauthorized();
    ...
    function _validateLaunchConfig(LaunchConfig calldata _launchConfig) internal view {
        if (_launchConfig.token1 == address(0)) {
            revert NajLaunchpad__InvalidTokens();
        }
        if (_launchConfig.token0 >= _launchConfig.token1) {
            revert NajLaunchpad__InvalidTokens();
        }
        if (_launchConfig.strategyAdapter == address(0)) {
            revert NajLaunchpad__InvalidStrategy();
        }
    }

    function _checkCurator(PoolId poolId) internal view {
        if (msg.sender != launchConfigs[poolId].curatorInfo.curator) revert NajLaunchpad__Unauthorized();
    }
}
